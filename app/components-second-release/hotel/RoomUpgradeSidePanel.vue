      ...
      <div v-if="isOpen" class="panel-overlay" @click.self="$emit('close')">
        <aside class="panel" @wheel.stop @touchmove.stop>
          ...
          <div class="panel__body">
            ...
            <!-- Upgrade rooms (incl. the optional 3-person room) -->
            <div
              v-for="room in extraRoomTypes"
              :key="room.id"
              class="room-card"
              :class="{
...
/** Every selectable non-base room: the optional 3-person room (auto
 *  inventory, +€50) followed by the regular paid upgrades. */
const extraRoomTypes = computed(() => store.allRoomTypes.filter(r => !r.isDefault))
