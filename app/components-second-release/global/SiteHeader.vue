  ...
  >
      ...
      <div class="site-header__nav-inner container">
        ...
        <NuxtLink :to="homeHref" class="site-header__logo">
          <!-- Both logos are always rendered; which one shows is decided by
               ...
               applies on home/search/deal/hotel — see sr-home-variants.css).
        ...
        <div class="site-header__nav-actions">
          ...
          <NuxtLink to="/second-release/leden" class="vip-btn">
              ...
              <Transition name="menu-slide">
                ...
                >
                  ...
                  <div class="menu-panel__body">
                    ...
                    <NuxtLink
                      v-if="effectiveNavVariant !== '1'"
                      to="/second-release/vakantieparken"
                      class="menu-panel__row menu-panel__row--lg"
                      @click="hamburgerDropdownOpen = false"
                    >
                      <span class="menu-panel__icon-tile">
                    ...
                    <NuxtLink
                      v-if="effectiveNavVariant === '1'"
                      to="/second-release/leden#aanmelden"
                      class="menu-panel__row menu-panel__row--sm"
                      @click="hamburgerDropdownOpen = false"
                    >
                      <span class="menu-panel__icon-tile menu-panel__icon-tile--sm">
    ...
    >
      ...
      <!-- Handwritten payoff under the search bar — shown ONLY in home
           layout variant 5 (gated in sr-home-variants.css), where it sits
    ...
    <Transition name="popup">
      <div v-if="activePopup" class="popup-backdrop" @click.self="onBackdropClick">
        ...
        <div
          ...
          style="width: 100%;"
        >
          <SecondReleaseDateAndDurationPopup
        ...
        <!-- WHO POPUP — adults + rooms steppers (no children) and a dog
             toggle that doubles as the "Honden welkom" filter. -->
        <div v-if="activePopup === 'who'" class="popup popup--who">
          ...
          <div class="who-row">
            ...
            <div class="stepper">
              ...
              <button class="stepper__btn" :disabled="searchGroup.adults >= 16" @click="searchGroup.adults++">+</button>
            </div>
          </div>

          <!-- Rooms — min ceil(persons/3), max = persons -->
          <div class="who-row">
            <div class="who-row__info">
              <span class="who-row__label">{{ t('travelGroup.rooms') }}</span>
            </div>
            <div class="stepper">
              <button class="stepper__btn" :disabled="searchGroup.rooms <= minRoomsFor(searchGroup.adults + searchGroup.children.length)" @click="searchGroup.rooms--">&#8722;</button>
          ...
          <!-- Dog — acts as the "Honden welkom" filter -->
          ...
          <div class="popup__footer">
            ...
            <button class="popup__done-btn" @click="applyWho()">{{ t('header.done') }}</button>
          </div>
        </div>
        </div>
      </div>
    ...
    <SecondReleaseMobileFullscreen :open="mobileMenuOpen" :title="'Menu'" @close="mobileMenuOpen = false">
      <nav class="mobile-menu">
        ...
        <div class="mobile-menu__section">
          ...
          <NuxtLink to="/second-release/vakantieparken" class="mobile-menu__item" @click="mobileMenuOpen = false">
            {{ t('header.holidayParks') }}
          </NuxtLink>
        </div>
        <div class="mobile-menu__divider"></div>
        <div class="mobile-menu__section">
          <NuxtLink to="/second-release/leden" class="mobile-menu__item" @click="mobileMenuOpen = false">
    ...
    <SecondReleaseMobileSearchModal
    ...
    <!-- Bottom-right floating hero-photo switcher. Only on the home
         ...
         photo hero. -->
    <SecondReleaseHeroPhotoSwitcher v-if="variant === 'overlay'" />
  </header>
</template>
...
import { tagsByCategory } from '~/utils-second-release/filterTags'
import { minRoomsFor, maxRoomsFor } from '~/utils-second-release/priceFormula'
import { useSecondReleaseHomeVariant } from '~/composables-second-release/useSecondReleaseHomeVariant'
import { useMobileSearchModalControl, useSearchNavLock } from '~/composables-second-release/useMobileSearchModalControl'
...
function onHamburgerClick() {
  ...
  // SecondReleaseMobileFullscreen drawer on mobile). Toggling the flag
...
useSecondReleaseClickOutside(langSwitcherRef, () => { langDropdownOpen.value = false })
useSecondReleaseClickOutside(phoneWrapRef, () => { phoneHoursOpen.value = false })
useSecondReleaseClickOutside(contactDropdownRef, () => { contactDropdownOpen.value = false })
...
const NO_PREF_KEY = 'vl_sr_no_dest_pref'
...
// Destination state lives in useSecondReleaseSearchState so /search and /kaart can read
...
/** The dog toggle doubles as the "Honden welkom" search filter. */
watch(() => searchGroup.value.dog, (dog) => {
  const active = selectedFilterTags.value.includes('dog-friendly')
  if (dog !== active) toggleFilterTag('dog-friendly')
})
...
function currentDealSlug(): string | null {
  ...
  // prefixed `/second-release/deal/<slug>` path layouts.
...
function handleSelectHotel(slug: string) {
  closePopup()
  navigateTo(`/second-release/deal/${slug}`)
}
...
/* The vertical (stacked) logo is only used by the V4 minimal mobile header
   — hidden everywhere by default; sr-home-variants.css reveals it (and hides
...
/* Handwritten payoff under the mobile search bar. Hidden everywhere by
   default; home layout variant 5 reveals it (see sr-home-variants.css). */
