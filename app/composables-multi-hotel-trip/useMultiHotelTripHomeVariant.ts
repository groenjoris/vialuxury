 ...
 * The prototype now ships a single homepage at `/multi-hotel-trip/home`
 * that always renders as "variant 1". `mhtNavVariant` is kept as a
...
type MhtNavVariant = '1' | '2' | '3' | '4' | '5' | '6' | '7'

const homeVariant = ref<HomeVariant>('1')
const mhtNavVariant = ref<MhtNavVariant>('1')
...
const STORAGE_KEY_HERO = 'vl_mht_hero_photo'
const heroPhotoIndex = ref(0)

/* ─────────── Mobile home/nav LAYOUT variant (1–4) ───────────
 * Independent of the legacy mhtNavVariant / data-mht-variant=6. Drives the
 * mobile-only home-hero + header layouts via `data-mht-home-variant` on
 * <html>; all variant CSS lives in app/assets/css/mht-home-variants.css. */
type HomeLayoutVariant = '1' | '2' | '3' | '4' | '5'
const STORAGE_KEY_HOME_VARIANT = 'vl_mht_home_variant'
...
if (import.meta.client) {
  watch(homeLayoutVariant, (v) => {
    ...
    document.documentElement.dataset.mhtHomeVariant = v
  }, { immediate: true })
}
...
if (import.meta.client) {
  watch(mhtNavVariant, () => {
    if (typeof document === 'undefined') return
    document.documentElement.dataset.mhtVariant = '6'
  }, { immediate: true })
}

export function useMultiHotelTripHomeVariant() {
  ...
  const homeHref = computed(() => '/multi-hotel-trip/home')

  /* ─────────── Nav-variant API (no-op; locked to '1') ─────────── */
  function setMhtNavVariant(_v: MhtNavVariant) {
