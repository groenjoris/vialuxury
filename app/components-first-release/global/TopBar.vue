<template>
  <!-- All FR variants hide the USP trust strip above the nav. -->
  <div v-if="!['1', '2', '3', '4', '5'].includes(frNavVariant)" class="top-bar">
    <div class="top-bar__inner container">
      ...
      <!-- Co-branded partner block (only when useFirstReleasePartner() is set) -->
...
const { partner, restore } = useFirstReleasePartner()
const { frNavVariant, restoreFrNavVariant } = useFirstReleaseHomeVariant()
onMounted(() => {
  restore()
  // Pick up the user's chosen variant on direct page loads so the TopBar
  // hide/show decision is correct immediately, not only after navigation.
