<!--
  Deprecated — use <HuisstijlDealCard> instead. The deal-first refactor replaced this
    ...
    <div class="result-card__content">
      ...
      <div class="result-card__hotel-header">
        <div class="result-card__hotel-info">
          <NuxtLink :to="`/huisstijl/hotel/${hotel.slug}`" target="_blank" class="result-card__name-link" @click.stop>
        ...
        <div class="result-card__score-wrap">
          <HuisstijlViaLuxuryScoreBadge :hotel="hotel" :all-hotels="mappedHotels" />
        </div>
      </div>
...
import { priceForArrival } from '~/utils-huisstijl/priceFormula'
import { nightsLabel, personsLabel } from '~/utils-huisstijl/plural'
...
const { persons, rooms, arrivalDate } = useHuisstijlSearchState()

// Label rendering moved to <HuisstijlDealLabel>; PNG/SVG lookup retired.
...
const singleDealHref = computed(() => {
  ...
  return `/huisstijl/deal/${isSingleDealLink.value}${q ? '?' + q : ''}`
})
</script>
