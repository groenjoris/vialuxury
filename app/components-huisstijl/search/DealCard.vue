  ...
  >
    ...
    <div class="deal-card-v2__content">
      ...
      <div v-if="showHotelInfo && hotel" class="deal-card-v2__hotel-info">
        <NuxtLink :to="`/huisstijl/hotel/${hotel.slug}`" target="_blank" class="deal-card-v2__name-link" @click.stop>
        ...
        <div ref="metaEl" class="deal-card-v2__meta">
          <HuisstijlViaLuxuryScoreBadge v-if="hotel" :hotel="hotel" :all-hotels="mappedHotels" />
...
import { formatDateShort } from '~/utils-huisstijl/formatDate'
import dayjs from 'dayjs'
import { pickSmartInclusions } from '~/utils-huisstijl/smartInclusions'
...
const dealHref = computed(() => {
  ...
  return `/huisstijl/deal/${props.deal.slug}${q ? '?' + q : ''}`
})
...
// Label rendering moved to <HuisstijlDealLabel>; the old PNG/SVG lookup is gone.
</script>
