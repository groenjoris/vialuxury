<template>
  <header class="site-header" :class="{ 'site-header--overlay': variant === 'overlay' }">
    ...
    <div class="site-header__nav">
      <div class="site-header__nav-inner container">
        ...
        <div class="site-header__nav-actions">
          ...
          <NuxtLink v-if="!isMobile" to="/huisstijl/leden" class="vip-btn">
          ...
          <div class="hamburger-wrap" ref="hamburgerWrapRef">
            ...
            <Teleport to="body">
              <div v-if="hamburgerDropdownOpen && !isMobile" class="hamburger-dropdown__menu" :style="hamburgerMenuStyle">
                  ...
                  <NuxtLink to="/huisstijl/vakantieparken" class="hamburger-dropdown__link" @click="hamburgerDropdownOpen = false">
                    {{ t('header.holidayParks') }}
                  </NuxtLink>

                  <div class="contact-dropdown__divider"></div>

                  <NuxtLink to="/huisstijl/leden" class="hamburger-dropdown__link" @click="hamburgerDropdownOpen = false">
    ...
    <HuisstijlMobileFullscreen :open="mobileMenuOpen" :title="'Menu'" @close="mobileMenuOpen = false">
      <nav class="mobile-menu">
        ...
        <div class="mobile-menu__section">
          ...
          <NuxtLink to="/huisstijl/vakantieparken" class="mobile-menu__item" @click="mobileMenuOpen = false">
            {{ t('header.holidayParks') }}
          </NuxtLink>
        </div>
        <div class="mobile-menu__divider"></div>
        <div class="mobile-menu__section">
          <NuxtLink to="/huisstijl/leden" class="mobile-menu__item" @click="mobileMenuOpen = false">
...
import { useHuisstijlLocaleStore } from '~/stores-huisstijl/locale'
import { searchHotels } from '~/data/mock/search-hotels'
import { tagsByCategory } from '~/utils-huisstijl/filterTags'
import { minRoomsFor, maxRoomsFor } from '~/utils-huisstijl/priceFormula'
import { useHuisstijlHomeVariant } from '~/composables-huisstijl/useHuisstijlHomeVariant'
...
const localeStore = useHuisstijlLocaleStore()
const { homeHref } = useHuisstijlHomeVariant()
...
useHuisstijlClickOutside(langSwitcherRef, () => { langDropdownOpen.value = false })
useHuisstijlClickOutside(contactDropdownRef, () => { contactDropdownOpen.value = false })
useHuisstijlClickOutside(hamburgerWrapRef, () => { hamburgerDropdownOpen.value = false })
...
// Destination state lives in useHuisstijlSearchState so /search and /kaart can read
...
/** Arrival date picker — live-synced with the global useHuisstijlSearchState.arrivalDate
...
watch(() => useHuisstijlSearchState().selectedFlexibility.value, (val) => {
...
function commitSearch() {
  ...
  navigateTo(fromSlug ? `/huisstijl/search?from=${encodeURIComponent(fromSlug)}` : '/huisstijl/search')
}
...
function handleSelectHotel(slug: string) {
  closePopup()
  navigateTo(`/huisstijl/deal/${slug}`)
}
