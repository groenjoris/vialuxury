    ...
    <div class="deal-card-v2__content">
      ...
      <div v-if="showHotelInfo && hotel" class="deal-card-v2__hotel-info">
        ...
        <h3 v-if="isTrip" class="deal-card-v2__name-row">
          <!-- Varianten "50-50" en "overlay": auto- of fietsicoon links van de titel
...
import { formatDateShort } from '~/utils-multi-hotel-trip/formatDate'
...
const dealHref = computed(() => {
  ...
  return `/multi-hotel-trip/deal/${props.deal.slug}${q ? '?' + q : ''}`
})
...
const mismatchHref = computed(() => {
  ...
  if (rooms.value !== 1) params.set('rooms', String(rooms.value))
  params.set('cal', '1')
  return `/multi-hotel-trip/deal/${props.deal.slug}?${params.toString()}`
})
...
// Label rendering moved to <MultiHotelTripDealLabel>; the old PNG/SVG lookup is gone.
</script>
