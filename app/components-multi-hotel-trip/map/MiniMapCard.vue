<template>
  <div class="mini-map">
    <NuxtLink
      :to="`/multi-hotel-trip/kaart?focus=${slug}`"
      class="mini-map__placeholder"
      :aria-label="t('common.viewMap')"
    >
      <MultiHotelTripStaticMiniMap :lat="lat" :lng="lng" :zoom="zoom" />
      <div class="mini-map__pin">
        <MultiHotelTripMapPin />
      </div>
    </NuxtLink>
    <div class="mini-map__footer">
      <span class="mini-map__address">{{ address }}</span>
      <NuxtLink :to="`/multi-hotel-trip/kaart?focus=${slug}`" class="mini-map__view-link">
        {{ t('common.viewMap') }}
      </NuxtLink>
    </div>
  </div>
