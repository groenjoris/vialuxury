<template>
  ...
  >
      ...
      <div class="site-header__nav-inner container">
        ...
        <NuxtLink ref="logoLinkRef" :to="homeHref" class="site-header__logo">
          <!-- Both logos are always rendered; which one shows is decided by
               ...
               applies on home/search/deal/hotel — see mht-home-variants.css).
        ...
        <!-- Opt-in pay-off under the logo (grid row 2, col 1 on desktop;
             ...
             /multi-hotel-trip/payoff-varianten page passes text + font to
        ...
        <div class="site-header__nav-actions">
          ...
          <NuxtLink to="/multi-hotel-trip/leden" class="vip-btn">
              ...
              <Transition name="menu-slide">
                ...
                >
                  ...
                  <div class="menu-panel__body">
                    ...
                    <NuxtLink
                      v-if="effectiveNavVariant !== '1'"
                      to="/multi-hotel-trip/vakantieparken"
                      class="menu-panel__row menu-panel__row--lg"
                      @click="hamburgerDropdownOpen = false"
                    >
                      <span class="menu-panel__icon-tile">
                    ...
                    <NuxtLink
                      to="/multi-hotel-trip/favorieten"
                      class="menu-panel__row menu-panel__row--lg"
                      @click="hamburgerDropdownOpen = false"
                    >
                      <span class="menu-panel__icon-tile">
                    ...
                    <NuxtLink
                      v-if="effectiveNavVariant === '1'"
                      to="/multi-hotel-trip/leden#aanmelden"
                      class="menu-panel__row menu-panel__row--sm"
                      @click="hamburgerDropdownOpen = false"
                    >
                      <span class="menu-panel__icon-tile menu-panel__icon-tile--sm">
    ...
    <div
      ...
      :class="{
        'site-header__mobile-search--summary': _route.path === '/multi-hotel-trip/search',
        'site-header__mobile-search--on-solid': variant !== 'overlay',
      }"
    >
      <MultiHotelTripMobileSearchSummary
        v-if="_route.path === '/multi-hotel-trip/search'"
      ...
      <!-- Handwritten payoff under the search bar — shown ONLY in home
           layout variant 5 (gated in mht-home-variants.css), where it sits
    ...
    <Transition name="popup">
      <div v-if="activePopup" class="popup-backdrop" @click.self="onBackdropClick">
        ...
        <div v-if="activePopup === 'destination'" class="popup popup--destination">
          <MultiHotelTripDestinationPopup
        ...
        <div
          ...
          style="width: 100%;"
        >
          <MultiHotelTripDateAndDurationPopup
    ...
    <MultiHotelTripMobileFullscreen :open="mobileMenuOpen" :title="'Menu'" @close="mobileMenuOpen = false">
      <nav class="mobile-menu">
        ...
        <div class="mobile-menu__section">
          <NuxtLink to="/multi-hotel-trip/leden" class="mobile-menu__item" @click="mobileMenuOpen = false">
    ...
    <MultiHotelTripMobileSearchModal
...
import { tagsByCategory } from '~/utils-multi-hotel-trip/filterTags'
...
import { useMobileSearchModalControl, useSearchNavLock } from '~/composables-multi-hotel-trip/useMobileSearchModalControl'
...
const localeStore = useMultiHotelTripLocaleStore()
const { homeHref, mhtNavVariant } = useMultiHotelTripHomeVariant()

/** Effective nav-bar layout — prop wins, otherwise read from the
 *  composable (which is restored from URL/localStorage on mount). */
const effectiveNavVariant = computed(() => props.navVariant ?? mhtNavVariant.value)
...
function onHamburgerClick() {
  ...
  // MultiHotelTripMobileFullscreen drawer on mobile). Toggling the flag
...
useMultiHotelTripClickOutside(langSwitcherRef, () => { langDropdownOpen.value = false })
useMultiHotelTripClickOutside(phoneWrapRef, () => { phoneHoursOpen.value = false })
useMultiHotelTripClickOutside(contactDropdownRef, () => { contactDropdownOpen.value = false })
...
const NO_PREF_KEY = 'vl_mht_no_dest_pref'
...
/** Arrival date picker — live-synced with the global useMultiHotelTripSearchState.arrivalDate
...
watch(() => useMultiHotelTripSearchState().selectedFlexibility.value, (val) => {
...
const BAR_SNAPSHOT_KEY = 'vl_mht_searchbar'
...
function currentDealSlug(): string | null {
  ...
  // prefixed `/multi-hotel-trip/deal/<slug>` path layouts.
...
function handleSelectHotel(slug: string) {
  closePopup()
  navigateTo(`/multi-hotel-trip/deal/${slug}`)
}
 ...
 *  truth lives in --mht-nav-row-h so it's tweakable from one line. */
.site-header--nav-v1,
.site-header--nav-v2,
.site-header--nav-v6 {
  --mht-nav-row-h: 44px;
...
/* The vertical (stacked) logo is only used by the V4 minimal mobile header
   — hidden everywhere by default; mht-home-variants.css reveals it (and hides
...
/* Handwritten payoff under the mobile search bar. Hidden everywhere by
   default; home layout variant 5 reveals it (see mht-home-variants.css). */
...
@media (max-width: 800px) {
  ...
  .site-header {
    ...
    --mht-mobile-logo-h: clamp(16px, 4.5vw, 25px);
    ...
    --mht-mobile-logo-w: clamp(156px, 43.9vw, 244px);
  }
