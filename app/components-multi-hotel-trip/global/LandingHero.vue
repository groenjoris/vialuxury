<template>
  <!-- Full-bleed hero shared by the homepage-style landing pages (Multi
       Hotel Trip: de Vakanties-zoekpagina). Wraps the overlay SiteHeader
       (which carries the nav + the search bar) over a background photo,
       with eyebrow / title / pitch copy. Overgenomen uit R2 (LandingHero). -->
  <section class="home-hero" :class="{ 'home-hero--search-below': searchBelow, 'home-hero--compact-title': compactTitle }">
    ...
    <MultiHotelTripSiteHeader variant="overlay">
      <template #hero>
...
withDefaults(defineProps<{
  ...
  bgUrl: string
  /** Push the cover-fit image down within its crop. */
  bgShiftDown?: boolean
  ...
  pitch: string
  /** Place the search bar BELOW the title/pitch instead of the homepage's
   *  top half-overlap dock. */
  searchBelow?: boolean
  /** Smaller title — for longer landing titles that would otherwise wrap
   *  and overflow the fixed-height hero. */
...
/* ── Landing (desktop): search bar BELOW the title/pitch. The hero becomes
   a flow column (nav → copy → search dock) instead of the homepage's
...
@media (min-width: 801px) {
  ...
  /* Landings have longer titles — shrink so they stay on one line and the
     copy + search bar fit inside the fixed 640px hero. */
