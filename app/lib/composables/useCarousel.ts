 ...
 *
 * The timer only runs when there is more than one item and `enabled`
 * (if provided) is truthy. It is cleared automatically on unmount.
 *
 * @param items Reactive list of items to cycle through.
 * @param options Optional `intervalMs` and `enabled` flag.
 * @returns `{ activeIndex, goTo }`.
 *
 * @example
 *   const images = computed(() => deal.value.images)
 *   const { activeIndex, goTo } = useCarousel(images, { enabled: isMultiRoom })
 */
