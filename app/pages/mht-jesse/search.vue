      ...
      <div class="search-page__grid container" :class="{ 'search-page__grid--no-sidebar': !sidebarVisible }">
        <Transition name="sidebar-slide">
          <div v-if="sidebarVisible" class="search-page__sidebar" ref="sidebarRef">
            ...
            <MhtJesseMapPreviewCard class="search-page__map-preview" @click="handleMapClick" />
            <MhtJesseSearchFilterPanel
        ...
        <div class="search-page__results">
          ...
          <div v-if="!isMobile" class="search-toolbar" :class="{ 'search-toolbar--trips': isTripMode }">
            ...
            <!-- Filter pills — sit in the toolbar between the left
                 ...
                 small height. -->
            <MhtJesseFilterPills v-if="!isTripMode" class="search-toolbar__pills" />
            ...
            <MhtJesseTripQuickFilters v-else inline :counts="tripQuickFilterCounts" />
          ...
          <template v-if="isMobile">
            ...
            <section class="search-page__mobile-pills">
              ...
              <MhtJesseTripQuickFilters v-if="isTripMode" :counts="tripQuickFilterCounts" />
              <MhtJesseFilterPills v-else :hide-location-date="true" />
            </section>
...
// /mht-jesse/vakanties is een eigen pagina (vakanties.vue) die deze
// component rendert — zie de toelichting daar (geen alias).
import type { SearchHotel } from '~/types/searchHotel'
import { searchHotels } from '~/data/mock/search-hotels'
import { dealMatchesAllTags, getFilterTag } from '~/utils-mht-jesse/filterTags'
import FilterPills from '~/components-mht-jesse/search/FilterPills.vue'
import { isDealAvailableInWindow } from '~/utils-mht-jesse/availability'
import { adjustPrice } from '~/utils-mht-jesse/priceFormula'
import {
...
} from '~/utils-mht-jesse/destinationMatch'
import { computeFilterCounts } from '~/utils-mht-jesse/filterCounts'
import { tripSearchHotels } from '~/data/mhtj-trips'
import { tripMatchesQuickFilters, TRIP_QUICK_FILTERS } from '~/utils-mht-jesse/tripFilters'
import { nightKeyFor, TRIP_NIGHT_KEYS } from '~/utils-mht-jesse/nights'
import { TRIPS_DESTINATION_ID } from '~/utils-mht-jesse/destinationMatch'
import { useMobileSearchModalControl } from '~/composables-mht-jesse/useMobileSearchModalControl'
...
function isSearchRoute(p: string): boolean {
  return p === '/mht-jesse/search' || p === '/mht-jesse/vakanties'
}

/* ── Multi Hotel Trip: vakantiestand ──
 * `isTripLanding` = we staan op /mht-jesse/vakanties (hero i.p.v. header,
...
const isTripLanding = computed(() => route.path === '/mht-jesse/vakanties')
...
const breadcrumbs = computed(() => [
  ...
  isTripMode.value
    ? { label: t('search.holidaysTitle'), href: '/mht-jesse/vakanties' }
...
onMounted(() => {
  ...
  window.addEventListener('resize', handleMobileToolbarScroll, { passive: true })
})
onUnmounted(() => {
  if (import.meta.client) {
    window.removeEventListener('scroll', handleMobileToolbarScroll)
    window.removeEventListener('resize', handleMobileToolbarScroll)
  }
})
...
// Budget range — shared with /kaart via useMhtJesseSearchState so toggling between
...
function navigateToDeal(slug: string) {
  ...
  const url = `/mht-jesse/deal/${slug}${qs ? '?' + qs : ''}`
