<template>
  <div class="deal-page">
    ...
    <main v-if="hotel && currentDeal" class="deal-page__main">
      ...
      <template v-if="isMobile">
        ...
        <section class="deal-page__breadcrumbs deal-page__breadcrumbs--with-actions container">
          <MhtJesseBreadcrumbNav :items="breadcrumbs" class="deal-page__breadcrumb-nav" />
          <div class="deal-page__mobile-actions">
            <div class="deal-page__share-wrap">
              ...
              <MhtJesseShareMenu :open="shareMenuOpen" @close="shareMenuOpen = false" />
            </div>
        ...
        <section class="deal-page__title-section deal-page__title-section--mobile container">
          ...
          <div class="deal-page__hotel-name-wrap">
            ...
            <NuxtLink v-else :to="`/mht-jesse/hotel/${hotel.slug}`" class="deal-page__hotel-link">
        ...
        <section v-if="creator && showCreatorCard" class="container deal-page__creator-mobile">
          <MhtJesseExperienceCreatorCard :creator="creator" />
        </section>
          ...
          <div class="deal-page__col-right deal-page__col-right--mobile">
            ...
            <ul class="sidebar__inc-list">
              <li v-for="inc in currentDeal.inclusions" :key="inc.id">
                ...
                <MhtJesseTripHotelText v-if="isTrip" :text="localized(inc.title)" :hotels="tripHotelLinks" @open-hotel="openTripHotel" />
        ...
        <section id="arrangement" class="container deal-page__content-blocks deal-page__content-blocks--mobile">
          ...
          <template v-if="isTrip">
            ...
            <MhtJesseTripItinerary :days="tripDaysView" :hotels="tripHotelLinks" stacked @open-hotel="openTripHotel" />
          </template>
          <template v-else>
        ...
        <div v-if="!isTrip" id="tips" class="deal-page__nearby-mobile">
          <MhtJesseHotelNearbyTips :tips="hotel.nearbyTips" :hotel-name="hotel.name" />
        </div>
        ...
        <section id="veelgestelde-vragen" class="container deal-page__faq deal-page__faq--mobile">
          <MhtJesseFaqSection :faq-items="hotel.faq" />
        </section>

        <!-- "Waarom ViaLuxury" (Tips moved up under Hotel faciliteiten). -->
        <MhtJesseWhyViaLuxury v-if="!isTrip" class="deal-page__why-mobile" />

        <!-- 17. Anderen bekeken ook -->
        <MhtJesseOthersAlsoViewed
      ...
      <section class="deal-page__breadcrumbs container">
        <MhtJesseBreadcrumbNav :items="breadcrumbs" />
      </section>
      ...
      <section class="deal-page__title-section container">
        ...
        >
          ...
          <MhtJesseExperienceCreatorCard v-if="showCreatorCard" :creator="creator" />
        </div>
      </section>
        ...
        <div class="deal-page__col-left">
          ...
          <section id="arrangement" class="deal-page__content-blocks">
            ...
            <template v-if="isTrip">
              <h2 class="section-title">{{ t('trip.itineraryHeading') }}</h2>
              <p class="deal-page__itinerary-intro">{{ t('trip.itineraryIntro') }}</p>
              <MhtJesseTripItinerary :days="tripDaysView" :hotels="tripHotelLinks" @open-hotel="openTripHotel" />
            </template>
            <template v-else>
            <h2 class="section-title">
              {{ t('deal.inclusionsHeading') }}
          ...
          <section v-if="isMobile" class="deal-page__mobile-faq">
            ...
            <MhtJesseFaqSection :faq-items="hotel.faq.slice(0, 3)" />
      ...
      <!-- "Waarom ViaLuxury" (Tips moved up under Hotel faciliteiten). -->
      <MhtJesseWhyViaLuxury v-if="!isTrip" class="deal-page__why-desktop" />
    ...
    <!-- Toast notification -->
    <MhtJesseToastNotification :message="toastMessage" type="success" />
    ...
    <MhtJesseMobileFullscreen :open="activeMobileSection === 'facilities'" :title="t('hotel.facilities')" @close="activeMobileSection = null">
    ...
    </MhtJesseMobileFullscreen>

    <MhtJesseMobileFullscreen :open="activeMobileSection === 'reviews'" :title="t('hotel.reviews')" @close="activeMobileSection = null">
    ...
    <MhtJesseMobileFullscreen :open="activeMobileSection === 'tips'" :title="t('hotel.nearbyTips') || 'Tips in de buurt'" @close="activeMobileSection = null">
      <div v-if="hotel" class="mobile-subpage mobile-subpage--no-padding">
        <MhtJesseHotelNearbyTips :tips="hotel.nearbyTips" :hotel-name="hotel.name" />
      </div>
    </MhtJesseMobileFullscreen>

    <MhtJesseMobileFullscreen :open="activeMobileSection === 'faq'" :title="t('hotel.faq') || 'Veelgestelde vragen'" @close="activeMobileSection = null">
      <div v-if="hotel" class="mobile-subpage">
        <MhtJesseFaqSection :faq-items="hotel.faq" />
      </div>
    </MhtJesseMobileFullscreen>
    ...
    <MhtJesseTripHotelPanel v-if="isTrip" overlay :hotel="tripPanelHotel" @close="tripPanelOpen = false" />
    <!-- Vakantie: fullscreen kaart met route, hotels en omgevingshighlights -->
    <MhtJesseTripFullscreenMap
      v-if="isTrip"
      :open="tripMapOpen"
...
import { facilityIcon } from '~/utils-mht-jesse/facilityIcon'
import { useMhtJesseDealStore } from '~/stores-mht-jesse/deal'
import { useSearchNavLock } from '~/composables-mht-jesse/useMobileSearchModalControl'
import { useBodyScrollLock } from '~/composables-mht-jesse/useBodyScrollLock'
import { usePinToViewportBottom } from '~/composables-mht-jesse/usePinToViewportBottom'
import { creatorForSlug } from '~/data/team-members'
import MhtJesseExperienceCreatorCard from '~/components-mht-jesse/deal/ExperienceCreatorCard.vue'
import MhtJesseWhyViaLuxury from '~/components-mht-jesse/deal/WhyViaLuxury.vue'
import MhtJesseOthersAlsoViewed from '~/components-mht-jesse/deal/OthersAlsoViewed.vue'
import { formatPrice } from '~/utils-mht-jesse/formatPrice'
import { getReviewLabelKey } from '~/utils-mht-jesse/reviewLabel'
import { generateDealAvailability } from '~/data/mock/deal-pricing'
import dayjs from 'dayjs'
import { formatDateWeekdayShort } from '~/utils-mht-jesse/formatDate'
import { tripPdpBySlug, tripHotelDetails } from '~/data/mhtj-trip-pdp'
import type { TripDayView, TripBlockView } from '~/components-mht-jesse/deal/TripItinerary.vue'
import type { TripHotelModalData } from '~/components-mht-jesse/deal/TripHotelDetails.vue'
import { PRICED_PERSONS, minRoomsFor } from '~/utils-mht-jesse/priceFormula'
import { matchIcon } from '~/utils-mht-jesse/iconMatcher'
import { roomsLeftForDeal } from '~/utils-mht-jesse/scarcity'
import { nightsLabel, nightsWord, personsLabel, roomsLabel } from '~/utils-mht-jesse/plural'
import {
...
const { t, localized, locale } = useMhtJesseI18n()
...
const store = useMhtJesseDealStore()
...
const { searchVersion } = useMhtJesseSearchState()
...
// Multi Hotel Trip: de boek-CTA leidt naar de checkout (room table uit het
// flexibel-annuleren prototype). Zonder gekozen datum eerst naar de
// kalenderstap; met datum direct naar de kamerkeuze. Op een telefoon stuurt
// de mhtj-mobile middleware door naar de mobiele checkout-site.
// Verse start vanaf de dealpagina: eerdere kalenderkeuze wissen.
useState<{ price: number } | null>('mhtj-checkout-day', () => null).value = null
function goToCheckout() {
  navigateTo(store.checkInDate ? '/mht-jesse/checkout/kamers' : '/mht-jesse/checkout/datum')
}
...
// Multi Hotel Trip: een vakantie-slug (zie data/mhtj-trips.ts) levert een
...
const { partner } = useMhtJessePartner()
...
/** Dagprogramma voor <MhtJesseTripItinerary>: de vaste blokken (inchecken,
...
const highlights = computed(() => {
  // Vakantie: de highlights uit de redactionele inhoud (mhtj-trip-itineraries.ts).
  const source = isTrip
...
const breadcrumbs = computed(() => [
  ...
  isTrip
    ? { label: t('header.holidays'), href: '/mht-jesse/vakanties' }
    : { label: t('search.arrangements'), href: '/mht-jesse/search' },
...
/** Mobile gallery "Ik ga boeken" → same checkout entry as the sidebar CTA. */
...
// `restoreMhtNavVariant` runs on mount so localStorage drives the nav
// chrome variant for direct-link visits.
const { restoreMhtNavVariant } = useMhtJesseHomeVariant()
const { showCreatorCard, restoreShowCreatorCard } = useMhtJesseCreatorCard()
onMounted(() => {
  restoreMhtNavVariant(window.location.pathname)
...
/* ===== 2-COLUMN GRID ===== */
.deal-page__grid { display: grid; grid-template-columns: 1fr var(--mhtj-deal-sidebar-width, 340px); gap: var(--space-xl); padding-top: var(--space-lg); align-items: start; }
...
/* Full description modal — all styling is shared with the hotel page and
   lives in app/assets/css/mhtj-variant-6.css (`.desc-modal*`). Keeping it in
...
/* Sidebar price ROW uses the shared MhtJesseStickyPriceRow
...
/* NOTE: the price ROW (vanaf/chip · original · amount · icon) and
   ...
   `MhtJesseStickyPriceRow` component — single source of truth,
