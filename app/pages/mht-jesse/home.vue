...
function goPartnerActie(partner: string) {
  ...
  clearDuration()
  resetBudget()
  navigateTo({ path: '/mht-jesse/search', query: { partner } })
}
...
const { setMhtNavVariant, heroPhotoUrl, heroPhotoIndex, restoreHeroPhotoIndex, restoreHomeLayoutVariant } = useMhtJesseHomeVariant()
onMounted(() => { setMhtNavVariant('1'); restoreHeroPhotoIndex(); restoreHomeLayoutVariant() })
</script>
...
@media (max-width: 800px) {
  ...
  /* Mobile home layout reorder (search bar below the hero copy) is
     variant-gated in app/assets/css/mhtj-home-variants.css — variant 1
