<template>
  <div class="home">
    ...
    <section class="home-hero">
      ...
      <FirstReleaseSiteHeader variant="overlay" nav-variant="5">
        <template #hero>
...
onMounted(() => { setFrNavVariant('5'); restoreHeroPhotoIndex() })
</script>
...
/* v5 inherits v4's hero layout: search bar at the TOP (right under
   the nav), hero text at the bottom. The folder tabs hug the top of
   the bar. */
.home-hero :deep(.site-header--overlay .site-header__search-dock) {
  margin-top: 0;
}

/* Tabs sit DIRECTLY above the search bar with 0 px gap. Nav's own
   padding-bottom would otherwise keep them 20 px above the bar, so
   zero it out for v5. The verticals also get a −1 px margin-bottom so
   their flat lower edges overlap the bar's top border by 1 px,
   collapsing the shared line. */
.home-hero :deep(.site-header--nav-v5 .site-header__nav) {
  padding-bottom: 0;
}
.home-hero :deep(.site-header--nav-v5 .verticals) {
  /* No padding / max-width — the parent `.site-header__nav-inner.container`
     already supplies the same alignment as the search-dock's container,
     so the leftmost tab lines up with the bar. */
  /* margin-top + margin-bottom now live in the SiteHeader's global v5
     rules so internal pages share the same bar Y. */
  display: flex;
...
/* v5 only — search bar gets a 1-px stroke and a flat top-left corner
   so the leftmost tab connects cleanly into it. */
.home-hero :deep(.site-header--nav-v5 .search-bar) {
...
.home-hero__content {
  ...
  gap: 28px;
  padding-bottom: 32px;
  /* v5 mirrors v4: hero text sits at the BOTTOM of the hero so the
     search bar can live up top. */
  position: absolute;
  bottom: 32px;
  top: auto;
  left: 0;
