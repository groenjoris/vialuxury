<template>
  <div class="deal-page">
    ...
    <main v-if="hotel && currentDeal" class="deal-page__main">
      ...
      <template v-if="isMobile">
        ...
        <section class="deal-page__breadcrumbs deal-page__breadcrumbs--with-actions container">
          <MultiHotelTripBreadcrumbNav :items="breadcrumbs" class="deal-page__breadcrumb-nav" />
          <div class="deal-page__mobile-actions">
            <div class="deal-page__share-wrap">
              ...
              <MultiHotelTripShareMenu :open="shareMenuOpen" @close="shareMenuOpen = false" />
            </div>
        ...
        <section id="veelgestelde-vragen" class="container deal-page__faq deal-page__faq--mobile">
          <MultiHotelTripFaqSection :faq-items="hotel.faq" />
        </section>
      ...
      <section class="deal-page__title-section container">
        ...
        >
          ...
          <MultiHotelTripExperienceCreatorCard v-if="showCreatorCard" :creator="creator" />
        </div>
      </section>
      ...
      <template v-if="isTrip">
        <section class="container deal-page__inc-row">
          <div class="inc-row">
      ...
      <div class="deal-page__grid container">
        <div class="deal-page__col-left">
          ...
          <section v-if="isMobile" class="deal-page__mobile-faq">
            ...
            <MultiHotelTripFaqSection :faq-items="hotel.faq.slice(0, 3)" />
    ...
    <MultiHotelTripHotelDealsSidePanel
    ...
    />
    <MultiHotelTripPackageSidePanel
    ...
    <!-- Toast notification -->
    <MultiHotelTripToastNotification :message="toastMessage" type="success" />
    ...
    <MultiHotelTripMobileFullscreen :open="activeMobileSection === 'facilities'" :title="t('hotel.facilities')" @close="activeMobileSection = null">
    ...
    <MultiHotelTripMobileFullscreen :open="activeMobileSection === 'reviews'" :title="t('hotel.reviews')" @close="activeMobileSection = null">
    ...
    <MultiHotelTripMobileFullscreen :open="activeMobileSection === 'tips'" :title="t('hotel.nearbyTips') || 'Tips in de buurt'" @close="activeMobileSection = null">
      <div v-if="hotel" class="mobile-subpage mobile-subpage--no-padding">
        <MultiHotelTripHotelNearbyTips :tips="hotel.nearbyTips" :hotel-name="hotel.name" />
      </div>
    </MultiHotelTripMobileFullscreen>

    <MultiHotelTripMobileFullscreen :open="activeMobileSection === 'faq'" :title="t('hotel.faq') || 'Veelgestelde vragen'" @close="activeMobileSection = null">
      <div v-if="hotel" class="mobile-subpage">
        <MultiHotelTripFaqSection :faq-items="hotel.faq" />
      </div>
    </MultiHotelTripMobileFullscreen>
...
import { useMultiHotelTripDealStore } from '~/stores-multi-hotel-trip/deal'
...
import MultiHotelTripOthersAlsoViewed from '~/components-multi-hotel-trip/deal/OthersAlsoViewed.vue'
...
const store = useMultiHotelTripDealStore()
const calendarRef = ref<HTMLElement | null>(null)
// Session-wide favourites (no login popup), keyed by hotel slug.
const { isFavorite: isFav, toggle: toggleFav } = useMultiHotelTripFavorites()
...
const { partner } = useMultiHotelTripPartner()
...
const { showCreatorCard, restoreShowCreatorCard } = useMultiHotelTripCreatorCard()
onMounted(() => {
...
/* Full description modal — all styling is shared with the hotel page and
   lives in app/assets/css/mht-variant-6.css (`.desc-modal*`). Keeping it in
...
/* Sidebar price ROW uses the shared MultiHotelTripStickyPriceRow
...
/* Grijs vlak dat de linkerkolom van rand tot rand vult (lijnt links en rechts
   uit met de rest van de pagina); de inhoud schuift door de padding naar binnen. */
.trip-incl {
  scroll-margin-top: 88px;
  margin-bottom: 40px;
...
/* NOTE: the price ROW (vanaf/chip · original · amount · icon) and
   ...
   `MultiHotelTripStickyPriceRow` component — single source of truth,
