...
function goPartnerActie(partner: string) {
  ...
  clearDuration()
  resetBudget()
  navigateTo({ path: '/multi-hotel-trip/search', query: { partner } })
}
...
const { setMhtNavVariant, heroPhotoUrl, heroPhotoIndex, restoreHeroPhotoIndex, restoreHomeLayoutVariant } = useMultiHotelTripHomeVariant()
...
@media (max-width: 800px) {
  ...
  /* Mobile home layout reorder (search bar below the hero copy) is
     variant-gated in app/assets/css/mht-home-variants.css — variant 1
