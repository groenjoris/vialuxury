<template>
  <div class="deal-page">
    ...
    <main v-if="hotel && currentDeal" class="deal-page__main">
      ...
      <template v-if="isMobile">
        ...
        <section class="deal-page__breadcrumbs deal-page__breadcrumbs--with-actions container">
          <SecondReleaseBreadcrumbNav :items="breadcrumbs" class="deal-page__breadcrumb-nav" />
          <div class="deal-page__mobile-actions">
            <div class="deal-page__share-wrap">
              ...
              <SecondReleaseShareMenu :open="shareMenuOpen" @close="shareMenuOpen = false" />
            </div>
        ...
        <section class="deal-page__title-section deal-page__title-section--mobile container">
          ...
          <div class="deal-page__hotel-name-wrap">
            <NuxtLink :to="`/second-release/hotel/${hotel.slug}`" class="deal-page__hotel-link">
        ...
        <section class="deal-page__sidebar-mobile container">
          <div class="deal-page__col-right deal-page__col-right--mobile">
            ...
            <!-- Booking controls: inline kalender / aankomstdatum-veld +
                 reisgezelschap + reisduur (zelfde component als desktop). -->
            <SecondReleaseBookingControls
              :has-other-arrangements="hasOtherArrangements"
              @open-arrangements="arrangementsPanelOpen = true"
            />

            <!-- Jouw vakantie: summary -->
            <div class="sidebar__summary">
              <h3 class="sidebar__title sidebar__title--vacation">Jouw vakantie</h3>
              <div class="vacation__dates">
                <div class="vacation__date">
                  <span class="vacation__date-label">Aankomst</span>
                  <span class="vacation__date-val" :class="{ 'vacation__date-val--placeholder': !store.checkInDate }">{{ vacationCheckIn || '—' }}</span>
                </div>
                <div class="vacation__date">
                  <span class="vacation__date-label">Vertrek</span>
                  <span class="vacation__date-val" :class="{ 'vacation__date-val--placeholder': !store.checkInDate }">{{ vacationCheckOut || '—' }}</span>
                </div>
                <button v-if="store.checkInDate" class="sidebar__date-clear" @click="store.clearDates()">{{ t('calendar.clearDates') }}</button>
              </div>
              ...
              <SecondReleaseStickyPriceRow
                ...
                info-variant="deal"
              />
              <p class="sidebar__price-meta">{{ vacationMetaLabel }}</p>
              ...
              <SecondReleaseSidebarPaymentLogos />
            </div>
        ...
        <!-- Jouw arrangement in detail: hotel + kamers -->
        <section id="arrangement-mobile" class="container deal-page__arr-detail-mobile">
          <SecondReleaseArrangementDetail :hotel="hotel" :inclusions="displayedInclusions" @open-description="descriptionOpen = true" />
        </section>
        ...
        <div id="tips" class="deal-page__nearby-mobile">
          <SecondReleaseHotelNearbyTips :tips="hotel.nearbyTips" :hotel-name="hotel.name" />
        </div>
        ...
        <section id="veelgestelde-vragen" class="container deal-page__faq deal-page__faq--mobile">
          <SecondReleaseFaqSection :faq-items="hotel.faq" />
        </section>

        <!-- "Waarom ViaLuxury" (Tips moved up under Hotel faciliteiten). -->
        <SecondReleaseWhyViaLuxury class="deal-page__why-mobile" />

        <!-- 17. Anderen bekeken ook -->
        <SecondReleaseOthersAlsoViewed
      ...
      <!-- Flow section: the "Jouw vakantie" card floats right; every block
           ...
           own BFC, so blocks NEXT TO the sidebar narrow automatically and
           blocks that start BELOW the sidebar take the full width. -->
      <div class="container deal-page__flow">
        ...
        <div class="deal-page__flow-sidebar deal-page__col-right">
          ...
          <!-- Price breakdown (only when an upgrade adds a line) -->
          <div v-if="store.pricing.breakdown.length > 1" class="sidebar__breakdown">
            <div v-for="(item, idx) in store.pricing.breakdown" :key="idx" class="sidebar__breakdown-row" :class="{ 'sidebar__breakdown-row--upgrade': item.amount > 0 && idx > 0, 'sidebar__breakdown-row--discount': item.amount < 0 }">
              <span>{{ item.label }}</span>
              <span>{{ item.amount < 0 ? '- ' : '' }}{{ formatPrice(Math.abs(item.amount)) }}</span>
        ...
        <!-- Booking controls: aankomstdatum (inline kalender) +
             reisgezelschap + reisduur. Narrows next to the sidebar. -->
        <SecondReleaseBookingControls
          class="deal-page__flow-controls"
          :has-other-arrangements="hasOtherArrangements"
          @open-arrangements="arrangementsPanelOpen = true"
        />

        <!-- Anchor target for the "Jouw arrangement" tab. -->
        <div id="arrangement" class="deal-page__flow-anchor" aria-hidden="true"></div>

        <!-- Jouw arrangement in detail: hotel + room cards, then the
             inclusion banners. Each card is its own BFC: narrow while
             beside the floated sidebar, full width once wholly below it. -->
        <div class="deal-page__flow-cards">
          <SecondReleaseArrangementDetail :hotel="hotel" :inclusions="displayedInclusions" only="cards" @open-description="descriptionOpen = true" />
        </div>
        <div class="deal-page__flow-cards">
          <SecondReleaseArrangementDetail :hotel="hotel" :inclusions="displayedInclusions" only="banners" />
        </div>
      </div>
      ...
      <!-- "Waarom ViaLuxury" (Tips moved up under Hotel faciliteiten). -->
      <SecondReleaseWhyViaLuxury class="deal-page__why-desktop" />
    ...
    <SecondReleaseHotelDealsSidePanel
    ...
    <SecondReleaseMobileFullscreen :open="activeMobileSection === 'facilities'" :title="t('hotel.facilities')" @close="activeMobileSection = null">
    ...
    <SecondReleaseMobileFullscreen :open="activeMobileSection === 'reviews'" :title="t('hotel.reviews')" @close="activeMobileSection = null">
    ...
    <SecondReleaseMobileFullscreen :open="activeMobileSection === 'tips'" :title="t('hotel.nearbyTips') || 'Tips in de buurt'" @close="activeMobileSection = null">
      <div v-if="hotel" class="mobile-subpage mobile-subpage--no-padding">
        <SecondReleaseHotelNearbyTips :tips="hotel.nearbyTips" :hotel-name="hotel.name" />
      </div>
    </SecondReleaseMobileFullscreen>

    <SecondReleaseMobileFullscreen :open="activeMobileSection === 'faq'" :title="t('hotel.faq') || 'Veelgestelde vragen'" @close="activeMobileSection = null">
      <div v-if="hotel" class="mobile-subpage">
        <SecondReleaseFaqSection :faq-items="hotel.faq" />
      </div>
    </SecondReleaseMobileFullscreen>
...
import SecondReleaseWhyViaLuxury from '~/components-second-release/deal/WhyViaLuxury.vue'
import SecondReleaseOthersAlsoViewed from '~/components-second-release/deal/OthersAlsoViewed.vue'
import { formatPrice } from '~/utils-second-release/formatPrice'
import { getReviewLabelKey } from '~/utils-second-release/reviewLabel'
import { generateDealAvailability } from '~/data/mock/deal-pricing'
import { PRICED_PERSONS, minRoomsFor } from '~/utils-second-release/priceFormula'
import { matchIcon } from '~/utils-second-release/iconMatcher'
import { roomsLeftForDeal } from '~/utils-second-release/scarcity'
import { nightsLabel, nightsWord, personsLabel, roomsLabel } from '~/utils-second-release/plural'
import {
...
/** Concept icon for an inclusion chip — falls back to the green ✓ when
 *  the title can't be confidently classified. */
function incIconUrl(inc: { title: unknown }): string | null {
  return matchIcon(localized(inc.title as Parameters<typeof localized>[0])).iconUrl
}

/** "Jouw vakantie" card — full-month Dutch dates ("26 juni 2026"). */
function formatVacationDate(iso: string | null): string {
  if (!iso) return ''
  const d = new Date(iso)
  if (Number.isNaN(d.getTime())) return ''
  return new Intl.DateTimeFormat('nl-NL', { day: 'numeric', month: 'long', year: 'numeric' }).format(d)
}
const vacationCheckIn = computed(() => formatVacationDate(store.checkInDate))
const vacationCheckOut = computed(() => formatVacationDate(store.checkOutDate))

/** "Voor 2 nachten, 5 personen, 3 kamers" — reflects the chosen
 *  reisgezelschap (the price itself stays the 2-persons prototype price). */
const vacationMetaLabel = computed(() => {
  const deal = currentDeal.value
  if (!deal) return ''
  const n = deal.nights
  const p = store.totalPersons
  const r = store.travelGroup.rooms
  return `Voor ${n} ${n === 1 ? 'nacht' : 'nachten'}, ${p} ${p === 1 ? 'persoon' : 'personen'}, ${r} ${r === 1 ? 'kamer' : 'kamers'}`
})
...
const { searchVersion } = useSecondReleaseSearchState()
...
const { partner } = useSecondReleasePartner()
...
const breadcrumbs = computed(() => [
  ...
  { label: t('search.arrangements'), href: '/second-release/search' },
...
/* Full description modal — all styling is shared with the hotel page and
   lives in app/assets/css/sr-variant-6.css (`.desc-modal*`). Keeping it in
...
/* Sidebar price ROW uses the shared SecondReleaseStickyPriceRow
...
/* NOTE: the price ROW (vanaf/chip · original · amount · icon) and
   ...
   `SecondReleaseStickyPriceRow` component — single source of truth,
...
/* ===== WIREFRAME REDESIGN (second release) ===== */

/* Inclusions — bordered chips in 2 columns */
.deal-page__inc-chips-section .section-title {
  margin-bottom: var(--space-md);
}
.inc-chips {
  display: grid;
...
/* Named-area grid — pairs the left sections with the right rail:
     chips    | nearby      (headings on the same level)
     controls | vacation    (tops align; vacation spans down)
     detail   | (vacation overflow)
   The left column flows naturally: with the calendar collapsed the
   detail section moves up to fill the space while "Jouw vakantie"
   stays anchored at the controls row. */
@media (min-width: 801px) {
  .deal-page__grid--areas {
    display: grid;
    /* Sidebar = 370px, the same width as the deal cards. */
    grid-template-columns: minmax(0, 1fr) 370px;
    grid-template-areas: "chips nearby";
    column-gap: var(--space-xl);
    align-items: start;
  }
  .ga-chips { grid-area: chips; margin-bottom: var(--space-xl); }
  .ga-nearby {
    grid-area: nearby;
    margin-bottom: var(--space-xl);
    /* Stretch with the includes block: the row is as tall as the
       chips grid, and the nearby block fills it — the map absorbs
       the flexible space between heading and tips. */
    align-self: stretch;
    display: flex;
    flex-direction: column;
  }
  .ga-nearby :deep(.mini-map) {
    flex: 1;
    min-height: 0;
  }
  .ga-nearby :deep(.mini-map__placeholder) {
    /* Height-driven instead of aspect-driven so the map scales with
       the chips column. Minimum = 3 chip rows (3×46px + 2×10px gap). */
    aspect-ratio: auto;
    flex: 1;
    min-height: 158px;
  }
  ...
  /* ── Flow section: floated sidebar + per-card BFC blocks ──
     Blocks beside the float narrow automatically; blocks that start
     wholly below the sidebar take the full container width. */
  .deal-page__flow {
    display: flow-root;
  }
  .deal-page__flow-sidebar {
    float: right;
    width: 370px;
    margin-left: var(--space-xl);
    margin-bottom: var(--space-lg);
  }
  .deal-page__flow-controls {
    display: flow-root;
    margin-bottom: var(--space-xl);
  }
  .deal-page__flow-anchor {
    height: 0;
    scroll-margin-top: 88px;
  }
  /* Unwrap the section wrappers so each .arr-card is a direct flow
     child (and thus narrows/widens individually). */
  .deal-page__flow-cards { display: contents; }
  .deal-page__flow-cards :deep(.arr-detail) { display: contents; }
  .deal-page__flow-cards :deep(.arr-detail__heading) { display: flow-root; }
}

/* "In de buurt" — plain block (no card stroke around the minimap);
   heading shares the .section-title style + spacing of the
   inclusions heading so both sit on the same visual level. */
.deal-page__nearby-block .section-title {
  margin-bottom: var(--space-md);
}
.nearby-card__tips {
  list-style: none;
...
/* "Jouw vakantie" — AANKOMST / VERTREK columns */
.vacation__dates {
  display: flex;
  align-items: flex-start;
  gap: var(--space-xl);
...
.vacation__date-val--placeholder {
  color: var(--color-text-muted);
  font-weight: 400;
}
.vacation__dates .sidebar__date-clear {
  margin-left: auto;
  align-self: flex-start;
}
.sidebar__title--vacation {
  margin-top: var(--space-md);
}

/* Mobile: arrangement-detail section spacing */
.deal-page__arr-detail-mobile {
  padding-top: 0;
}
@media (max-width: 800px) {
  .deal-page__arr-detail-mobile :deep(.arr-detail) {
    margin-top: var(--space-md);
  }
}
</style>
