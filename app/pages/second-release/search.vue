      ...
      <div class="search-page__grid container" :class="{ 'search-page__grid--no-sidebar': !showFilters }">
        <Transition name="sidebar-slide">
          <div v-if="showFilters" class="search-page__sidebar" ref="sidebarRef">
            ...
            <SecondReleaseMapPreviewCard class="search-page__map-preview" @click="handleMapClick" />
        ...
        <div class="search-page__results">
          ...
          <div v-if="!isMobile" class="search-toolbar">
            ...
            <!-- Filter pills — sit in the toolbar between the left
                 ...
                 small height. -->
            <SecondReleaseFilterPills class="search-toolbar__pills" />
          ...
          <template v-if="isMobile">
            ...
            <section class="search-page__mobile-pills">
              ...
              <SecondReleaseFilterPills :hide-location-date="true" />
            </section>
...
} from '~/utils-second-release/destinationMatch'
import { computeFilterCounts } from '~/utils-second-release/filterCounts'
import { useMobileSearchModalControl } from '~/composables-second-release/useMobileSearchModalControl'
...
const activeArrival = computed(() =>
  route.path === '/second-release/search' ? liveArrivalDate.value : committedArrivalDate.value,
)
const activeFlex = computed(() =>
  route.path === '/second-release/search' ? liveFlexibility.value : committedFlexibility.value,
)
...
// from useSecondReleaseSearchState.
...
// Budget range — shared with /kaart via useSecondReleaseSearchState so toggling between
...
function handleMapClick() {
  navigateTo('/second-release/kaart')
}
...
function navigateToDeal(slug: string) {
  ...
  const url = `/second-release/deal/${slug}${qs ? '?' + qs : ''}`
