  ...
  <div class="hotel-page">
    ...
    <main v-if="hotel" class="hotel-page__main">
      ...
      <section class="hotel-page__breadcrumbs container" :class="{ 'hotel-page__breadcrumbs--with-actions': isMobile }">
        <SecondReleaseBreadcrumbNav :items="breadcrumbs" :class="{ 'hotel-page__breadcrumb-nav': isMobile }" />
      ...
      <section id="arrangementen" class="hotel-page__deals container">
        ...
        <div v-if="!isMobile" class="hotel-page__deals-search">
          ...
          <SecondReleaseHotelSearchBar ref="searchBarRef" />
        </div>
    ...
    <SecondReleaseToastNotification />
...
import { nightsLabel, personsLabel, roomsLabel } from '~/utils-second-release/plural'
...
import DealCard from '~/components-second-release/search/DealCard.vue'
...
/* Heart + share live inside `.hotel-page__tabs` (right-aligned via
   `.hotel-page__tabs-actions` in sr-variant-6.css), same row as the
...
/* Full description modal — all styling is shared with the deal page and
   lives in app/assets/css/sr-variant-6.css (`.desc-modal*`). */
