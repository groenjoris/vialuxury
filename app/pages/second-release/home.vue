...
import { POPULAR_FILTER_ICONS } from '~/utils-second-release/popularFilterIcons'
import { mappedHotels } from '~/data/deals-mapper'
import type { SearchHotel } from '~/types/searchHotel'
import { pickPrimaryDeal } from '~/utils-second-release/primaryDeal'
...
import { FILTER_TAGS } from '~/utils-second-release/filterTags'
...
function goPartnerActie(partner: string) {
  ...
  clearDuration()
  resetBudget()
  navigateTo({ path: '/second-release/search', query: { partner } })
}
...
const { setFrNavVariant, heroPhotoUrl, heroPhotoIndex, restoreHeroPhotoIndex, restoreHomeLayoutVariant } = useSecondReleaseHomeVariant()
...
@media (max-width: 800px) {
  ...
  /* Mobile home layout reorder (search bar below the hero copy) is
     variant-gated in app/assets/css/sr-home-variants.css — variant 1
