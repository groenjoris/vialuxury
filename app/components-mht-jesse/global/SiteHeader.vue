<template>
  ...
  >
      ...
      <div class="site-header__nav-inner container">
        ...
        <NuxtLink ref="logoLinkRef" :to="homeHref" class="site-header__logo">
          <!-- Both logos are always rendered; which one shows is decided by
               ...
               applies on home/search/deal/hotel — see mhtj-home-variants.css).
        ...
        <!-- Opt-in pay-off under the logo (grid row 2, col 1 on desktop;
             ...
             /mht-jesse/payoff-varianten page passes text + font to
          ...
          <Transition name="dropdown">
            <div v-if="phoneHoursOpen" class="site-header__phone-popover" role="dialog" aria-label="Openingstijden">
              ...
              <a href="tel:+31207052222" class="site-header__phone-popover-item">
                ...
                <span class="site-header__phone-popover-item-sub">{{ t('header.phone') }}</span>
              </a>
        ...
        <div class="site-header__nav-actions">
          ...
          <NuxtLink to="/mht-jesse/leden" class="vip-btn">
    ...
    <div
      ...
      :class="{
        'site-header__mobile-search--summary': _route.path === '/mht-jesse/search',
        'site-header__mobile-search--on-solid': variant !== 'overlay',
      }"
    >
      <MhtJesseMobileSearchSummary
        v-if="_route.path === '/mht-jesse/search'"
      ...
      <!-- Handwritten payoff under the search bar — shown ONLY in home
           layout variant 5 (gated in mhtj-home-variants.css), where it sits
      ...
      <div v-if="activePopup" class="popup-backdrop" @click.self="onBackdropClick">
        ...
        <div v-if="activePopup === 'who'" class="popup popup--who popup--who-mvp">
          <ul class="who-mvp__list" role="listbox" aria-label="Aantal personen">
            ...
            >
              <span>{{ opt.adults }} {{ opt.adults === 1 ? t('common.personSingular') : t('common.personPlural') }} / {{ opt.rooms }} {{ opt.rooms === 1 ? t('common.roomSingular') : t('common.roomPlural') }}</span>
              <svg
    ...
    <MhtJesseMobileFullscreen :open="mobileMenuOpen" :title="'Menu'" @close="mobileMenuOpen = false">
      <nav class="mobile-menu">
        ...
        <div class="mobile-menu__section">
          ...
          <NuxtLink to="/mht-jesse/vakantieparken" class="mobile-menu__item" @click="mobileMenuOpen = false">
            {{ t('header.holidayParks') }}
          </NuxtLink>
        </div>
        <div class="mobile-menu__divider"></div>
        <div class="mobile-menu__section">
          <NuxtLink to="/mht-jesse/leden" class="mobile-menu__item" @click="mobileMenuOpen = false">
    ...
    <!-- Bottom-right floating hero-photo switcher. Only on the home
         ...
         photo hero. -->
    <MhtJesseHeroPhotoSwitcher v-if="variant === 'overlay'" />
  </header>
</template>

<script setup lang="ts">
import { TRIP_NIGHT_KEYS, isNightKey, nightKeyFor, joinNightKeys, summarizeNightKeys } from '~/utils-mht-jesse/nights'
import { TRIPS_DESTINATION_ID } from '~/utils-mht-jesse/destinationMatch'
import { useMhtJesseLocaleStore } from '~/stores-mht-jesse/locale'
import { searchHotels } from '~/data/mock/search-hotels'
import { mappedHotels } from '~/data/deals-mapper'
import { tagsByCategory } from '~/utils-mht-jesse/filterTags'
import { minRoomsFor, maxRoomsFor } from '~/utils-mht-jesse/priceFormula'
import { useMhtJesseHomeVariant } from '~/composables-mht-jesse/useMhtJesseHomeVariant'
import { useMobileSearchModalControl, useSearchNavLock } from '~/composables-mht-jesse/useMobileSearchModalControl'
...
const localeStore = useMhtJesseLocaleStore()
const { homeHref, mhtjNavVariant } = useMhtJesseHomeVariant()

/** Effective nav-bar layout — prop wins, otherwise read from the
 *  composable (which is restored from URL/localStorage on mount). */
const effectiveNavVariant = computed(() => props.navVariant ?? mhtjNavVariant.value)
...
const verticals = computed(() => {
  const all = [
    ...
    { id: 'vakanties', label: t('header.holidays'), href: '/mht-jesse/vakanties', external: false, badge: t('header.new') },
...
/** Nights key for the duration picker ('1' … '8', zie utils-mht-jesse/nights.ts). */
const dealNightsKey = dealPageDeal ? nightKeyFor(dealPageDeal.nights) : null
const activeVertical = computed(() => {
  if (_route.path.startsWith('/mht-jesse/vakanties')) return 'vakanties'
...
function onHamburgerClick() {
  ...
  // MhtJesseMobileFullscreen drawer on mobile). Toggling the flag
...
useMhtJesseClickOutside(langSwitcherRef, () => { langDropdownOpen.value = false })
useMhtJesseClickOutside(phoneWrapRef, () => { phoneHoursOpen.value = false })
useMhtJesseClickOutside(contactDropdownRef, () => { contactDropdownOpen.value = false })
...
const NO_PREF_KEY = 'vl_mhtj_no_dest_pref'
...
// Destination state lives in useMhtJesseSearchState so /search and /kaart can read
...
/** Arrival date picker — live-synced with the global useMhtJesseSearchState.arrivalDate
...
watch(() => useMhtJesseSearchState().selectedFlexibility.value, (val) => {
...
const BAR_SNAPSHOT_KEY = 'vl_mhtj_searchbar'
...
async function commitSearch() {
  ...
  const target = localDestDestinations.value.includes(TRIPS_DESTINATION_ID)
    ? '/mht-jesse/vakanties'
    : fromSlug
      ? `/mht-jesse/search?from=${encodeURIComponent(fromSlug)}`
      : '/mht-jesse/search'
...
function enterTrips() {
  ...
  saveBarSnapshot()
  triggerSearchUpdate()
  navigateTo('/mht-jesse/vakanties')
}
...
function currentDealSlug(): string | null {
  ...
  // prefixed `/mht-jesse/deal/<slug>` path layouts.
...
function handleSelectHotel(slug: string) {
  closePopup()
  navigateTo(`/mht-jesse/deal/${slug}`)
}
 ...
 *  truth lives in --mhtj-nav-row-h so it's tweakable from one line. */
.site-header--nav-v1,
.site-header--nav-v2,
.site-header--nav-v6 {
  --mhtj-nav-row-h: 44px;
...
.site-header--nav-v6 .hamburger-btn {
  width: var(--mhtj-nav-row-h);
  height: var(--mhtj-nav-row-h);
}

/* ─── Variant 1 / 6 — top-align the logo with the row-1 buttons (which
 *  define row 1 height at --mhtj-nav-row-h). The logo is shorter than the
...
/* The vertical (stacked) logo is only used by the V4 minimal mobile header
   — hidden everywhere by default; mhtj-home-variants.css reveals it (and hides
...
/* Handwritten payoff under the mobile search bar. Hidden everywhere by
   default; home layout variant 5 reveals it (see mhtj-home-variants.css). */
...
@media (max-width: 800px) {
  ...
  .site-header {
    ...
    --mhtj-mobile-logo-h: clamp(16px, 4.5vw, 25px);
    ...
    --mhtj-mobile-logo-w: clamp(156px, 43.9vw, 244px);
  }
