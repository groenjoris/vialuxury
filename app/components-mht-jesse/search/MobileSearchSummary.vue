...
import { summarizeNightKeys } from '~/utils-mht-jesse/nights'

const { t } = useMhtJesseI18n()
defineEmits<{ open: [] }>()

const search = useMhtJesseSearchState()
...
const whenCombinedLabel = computed<string>(() => {
  ...
  // Duration part — fallback "Elke reisduur": the pill has no field
  // labels, so the desktop's "Geen voorkeur" would read as meaningless here.
...
.mss {
  ...
  padding: 16px;
  background: #fff;
  border: 4px solid var(--color-primary, #FB862C);
...
.mss__pencil {
  ...
  color: var(--color-primary, #FB862C);
}
