  ...
  >
    ...
    <div class="deal-card-v2__image">
      ...
      <template v-if="isTrip">
        ...
        <MhtJesseTripRouteMap class="deal-card-v2__trip-map" :stops="tripMapStops" hoverable @stop-hover="tripHoverIndex = $event" />
      </template>
    ...
    <div class="deal-card-v2__content">
      ...
      <div v-if="showHotelInfo && hotel" class="deal-card-v2__hotel-info">
        ...
        <NuxtLink v-else :to="`/mht-jesse/hotel/${hotel.slug}`" :target="linkTarget" class="deal-card-v2__name-link" @click.stop>
...
import { formatDateShort } from '~/utils-mht-jesse/formatDate'
...
import { arrangementSuffixFromHighlights } from '~/utils-mht-jesse/arrangementType'
import { dealHash, roomsLeftForDeal } from '~/utils-mht-jesse/scarcity'
...
const mismatchHref = computed(() => {
  ...
  if (rooms.value !== 1) params.set('rooms', String(rooms.value))
  params.set('cal', '1')
  return `/mht-jesse/deal/${props.deal.slug}?${params.toString()}`
})
...
// Label rendering moved to <MhtJesseDealLabel>; the old PNG/SVG lookup is gone.
</script>
