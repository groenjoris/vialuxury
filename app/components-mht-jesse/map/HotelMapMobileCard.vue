...
import { nightsLabel, personsLabel } from '~/utils-mht-jesse/plural'
...
const { persons, arrivalDate, selectedFlexibility } = useMhtJesseSearchState()
...
function hrefFor(deal: SearchHotelDeal): string {
  ...
  return `/mht-jesse/deal/${deal.slug}${q ? '?' + q : ''}`
}
  ...
  return [...h.deals]
    ...
    .map((deal) => {
      ...
      if (soldOut) {
        ...
        p.set('cal', '1')
        href = `/mht-jesse/deal/${deal.slug}?${p.toString()}`
      } else {
        href = hrefFor(deal)
      }
      return {
