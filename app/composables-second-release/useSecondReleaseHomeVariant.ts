 ...
 * The prototype now ships a single homepage at `/second-release/home`
 * that always renders as "variant 1". `srNavVariant` is kept as a
...
const srNavVariant = ref<FrNavVariant>('1')
...
const STORAGE_KEY_HERO = 'vl_sr_hero_photo'
const heroPhotoIndex = ref(0)

/* ─────────── Mobile home/nav LAYOUT variant (1–4) ───────────
 * Independent of the legacy srNavVariant / data-sr-variant=6. Drives the
 * mobile-only home-hero + header layouts via `data-sr-home-variant` on
 * <html>; all variant CSS lives in app/assets/css/sr-home-variants.css. */
type HomeLayoutVariant = '1' | '2' | '3' | '4' | '5'
const STORAGE_KEY_HOME_VARIANT = 'vl_sr_home_variant'
const homeLayoutVariant = ref<HomeLayoutVariant>('1')

if (import.meta.client) {
  watch(homeLayoutVariant, (v) => {
    if (typeof document === 'undefined') return
    document.documentElement.dataset.srHomeVariant = v
  }, { immediate: true })
}
...
if (import.meta.client) {
  watch(srNavVariant, () => {
    if (typeof document === 'undefined') return
    document.documentElement.dataset.srVariant = '6'
  }, { immediate: true })
}

export function useSecondReleaseHomeVariant() {
  ...
  const homeHref = computed(() => '/second-release/home')
