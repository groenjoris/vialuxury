<template>
  <header class="site-header" :class="{ 'site-header--overlay': variant === 'overlay' }">
    ...
    <NorthstarMobileFullscreen :open="mobileMenuOpen" :title="'Menu'" @close="mobileMenuOpen = false">
...
import { useNorthstarLocaleStore } from '~/stores-northstar/locale'
import { searchHotels } from '~/data/mock/search-hotels'
import { tagsByCategory } from '~/utils-northstar/filterTags'
import { minRoomsFor, maxRoomsFor } from '~/utils-northstar/priceFormula'
...
const localeStore = useNorthstarLocaleStore()
...
useNorthstarClickOutside(langSwitcherRef, () => { langDropdownOpen.value = false })
useNorthstarClickOutside(contactDropdownRef, () => { contactDropdownOpen.value = false })
useNorthstarClickOutside(hamburgerWrapRef, () => { hamburgerDropdownOpen.value = false })
...
// Destination state lives in useNorthstarSearchState so /search and /kaart can read
...
/** Arrival date picker — live-synced with the global useNorthstarSearchState.arrivalDate
...
watch(() => useNorthstarSearchState().selectedFlexibility.value, (val) => {
