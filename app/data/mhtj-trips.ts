 ...
 *
 * De vakantie-PDP (/mht-jesse/deal/<slug>) leest `tripDetailBySlug`:
 * per hotel de dagen, ligging en de "Inclusief"-lijst uit de PDF; de
 * zijbalk toont de samengevatte `inclusions`.
 */
import type { LocalizedString } from '~/i18n/types'
import type { SearchHotel, SearchHotelDeal, MhtJesseType, MhtJesseStop } from '~/types/searchHotel'
...
export interface MhtJesseDetailStop extends MhtJesseStop {
  nights: number
  region: string
...
export interface MhtJesseDetail {
  id: string
  slug: string
  type: MhtJesseType
  title: LocalizedString
...
function toStop(s: StopSpec, dayFrom: number): MhtJesseDetailStop {
...
function buildTrip(spec: TripSpec): { hotel: SearchHotel; detail: MhtJesseDetail } | null {
  const stops: MhtJesseDetailStop[] = []
  let day = 1
  ...
  const pdpHref = `/mht-jesse/deal/${spec.slug}`
...
const built = TRIPS
  .map(buildTrip)
  .filter((t): t is { hotel: SearchHotel; detail: MhtJesseDetail } => t !== null)
...
export const tripDetailBySlug: Record<string, MhtJesseDetail> = Object.fromEntries(
