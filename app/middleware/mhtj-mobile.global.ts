...
// omgeleid naar /mht-jesse/m/checkout/...; de mobiele site blijft
...
export default defineNuxtRouteMiddleware((to) => {
  ...
  const match = to.path.match(/^\/mht-jesse\/checkout\/(datum|kamers)$/)
  if (!match) return
  ...
  if (!isMobile) return
  return navigateTo(`/mht-jesse/m/checkout/${match[1]}`, { replace: true })
})
