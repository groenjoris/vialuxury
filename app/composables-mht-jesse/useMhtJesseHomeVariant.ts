 ...
 * The prototype now ships a single homepage at `/mht-jesse/home`
 * that always renders as "variant 1". `mhtjNavVariant` is kept as a
...
type MhtjNavVariant = '1' | '2' | '3' | '4' | '5' | '6' | '7'

const homeVariant = ref<HomeVariant>('1')
const mhtjNavVariant = ref<MhtjNavVariant>('1')
...
const STORAGE_KEY_HERO = 'vl_mhtj_hero_photo'
const heroPhotoIndex = ref(0)

/* ─────────── Mobile home/nav LAYOUT variant (1–4) ───────────
 * Independent of the legacy mhtjNavVariant / data-mhtj-variant=6. Drives the
 * mobile-only home-hero + header layouts via `data-mhtj-home-variant` on
 * <html>; all variant CSS lives in app/assets/css/mhtj-home-variants.css. */
type HomeLayoutVariant = '1' | '2' | '3' | '4' | '5'
const STORAGE_KEY_HOME_VARIANT = 'vl_mhtj_home_variant'
...
if (import.meta.client) {
  watch(homeLayoutVariant, (v) => {
    ...
    document.documentElement.dataset.mhtjHomeVariant = v
  }, { immediate: true })
}
...
if (import.meta.client) {
  watch(mhtjNavVariant, () => {
    if (typeof document === 'undefined') return
    document.documentElement.dataset.mhtjVariant = '6'
  }, { immediate: true })
}

export function useMhtJesseHomeVariant() {
  ...
  const homeHref = computed(() => '/mht-jesse/home')

  /* ─────────── Nav-variant API (no-op; locked to '1') ─────────── */
  function setMhtNavVariant(_v: MhtjNavVariant) {
    // Variant switcher removed — value is permanently '1'. Kept as a
    // no-op so existing call-sites (`setMhtNavVariant('1')` in
    // onMounted hooks) compile without changes.
  }

  function restoreMhtNavVariant(_routePath?: string) {
    // No-op — see setMhtNavVariant.
  }
